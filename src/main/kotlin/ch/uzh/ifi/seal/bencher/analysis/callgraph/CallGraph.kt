...
data class CGResult(
    ...
    override fun reachable(from: Method, to: Method): Boolean {
        val mcs = calls[from] ?: return false
        return mcs.map { to == it.method }.reduce { acc, r -> acc || r }
    }
}
...
fun merge(cgr1: CGResult, cgr2: CGResult): CGResult {
    ...
    val newCalls = mutableMapOf<Method, Iterable<MethodCall>>()
